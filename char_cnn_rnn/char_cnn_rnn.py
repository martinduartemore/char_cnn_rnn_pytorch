...
def prepare_text(string):
    ...
    '''
    labels = string_to_label_vector(string)
    one_hot = label_vector_to_one_hot(labels)
    return one_hot



def string_to_label_vector(string):
    max_str_len = 201
    string = string.lower()
...
def label_vector_to_one_hot(labels):
    max_str_len = 201
    labels = torch.LongTensor(labels).unsqueeze(1)
    one_hot = torch.zeros(max_str_len, 71).scatter_(1, labels, 1.)
...
def txt_tensor_to_string(txt_tensor):
    '''
    Converts a text description from one-hot tensor format to string format.
    '''
    alphabet = "abcdefghijklmnopqrstuvwxyz0123456789-,;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]{} "
    string = [alphabet[x-1] for x in txt_tensor if x > 0]
    string = ''.join(string)
    return string
